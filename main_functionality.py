...
# get the audio devices from the PC and initializing the interface
devices = AudioUtilities.GetSpeakers()
interface = devices.Activate(IAudioEndpointVolume._iid_, -1, None)
volume = interface.QueryInterface(IAudioEndpointVolume)

# getting the range of volume
volMin, volMax = volume.GetVolumeRange()[:2]   #(minVolume, maxVolume, volumeStep, defaultVolume)
...
def process(image):
    ...
    # drawing hand annotations on vid
    image.flags.writeable = True
    image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
    
    # left and right hand landmarks to be stored
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            # the landmarks are stored in the List
            lmList = []
            ...
            # to check if hand is on right of frame
            if lmList and lmList[0][1] > w // 2:
                right_hand_landmarks = lmList
            elif lmList:
                left_hand_landmarks = lmList

    # to control volume based on left hand position
    if left_hand_landmarks:
        ...
        # to calc distance between fingers (origin)
        ...
        # t0 draw circles and lines on fingers
        cv2.circle(image, (x1, y1), 4, (255, 0, 0), cv2.FILLED)
        cv2.circle(image, (x2, y2), 4, (255, 0, 0), cv2.FILLED)
        cv2.line(image, (x1, y1), (x2, y2), (255, 0, 0), 3)
        
    # to control video playback based on right hand position
    if right_hand_landmarks:
        # to Extract coordinates of key points (fingers)
        ...
        # now we Calculate distances between fingers
        ...
        # Check if all distances are greater than a threshold given
